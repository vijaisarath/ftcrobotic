...
// @Disabled
public class AutonomousSquareEncoder extends LinearOpMode {
    ...
    private void moveFront(int power, int distanceInch) {
        FL = power;
        BL = power;
        FR = power;
        BR = power;

        int dist = (int) (distanceInch * COUNTS_PER_INCH);

        frontLeftMotor.setTargetPosition(dist);
        frontRightMotor.setTargetPosition(dist);
        backLeftMotor.setTargetPosition(dist);
        backRightMotor.setTargetPosition(dist);
        ...
        while (opModeIsActive() &&
            ...
            telemetry.addData("Path1", "Running to %7d", dist);
